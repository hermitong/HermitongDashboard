...
1. 在项目目录下创建 `TradeRecord` 文件夹 (如果不存在)
2. 将券商导出的 Excel 交易记录文件放入该文件夹
3. 确保文件格式为 `.xlsx`

**Excel 文件要求**:
- 必须包含以下列: `Order Status`, `Symbol`, `Direction`, `Order Qty`, `Avg Price`, `Order Time`
- `Order Status` 列中必须有 `已成交` 状态的订单
...
1. 从券商导出新的交易记录 Excel 文件
2. 将文件放入 `TradeRecord` 文件夹
3. 运行脚本:
   ```bash
   python3 process_files.py
   ```
4. 脚本会自动识别新文件并处理
5. 刷新 Looker 仪表盘查看更新后的数据
...
目前脚本针对特定格式设计,如果你的券商格式不同,需要修改 `clean_and_transform_orders()` 函数中的列名映射。
